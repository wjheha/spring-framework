	...
	public final TransactionStatus getTransaction(@Nullable TransactionDefinition definition)
		...
		if (def.getPropagationBehavior() == TransactionDefinition.PROPAGATION_MANDATORY) {
			...
			try {
				...
				doBegin(transaction, def);    // **doBegin**构造事务，包括设置connectionHolder，隔离级别，timeout，如果是新连接，绑定到当前线程
				prepareSynchronization(status, def);     //新同步事务的设置，针对当前线程的设置，这里用到了ThreadLocal哦。
				return status;
			}
		...
		try {
			...
			try {
				...
				if (status.hasSavepoint()) {
					...
					// 如果当前事务为独立的新事务，则直接回退，直接执行数据库的callback回滚
					doRollback(status);
				}
				else {
			...
			// 清空前面记录的资源，并将挂起的线程恢复，前面有挂起，这里有恢复哦！
			cleanupAfterCompletion(status);
		}
	}
