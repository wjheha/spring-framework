 ...
 */
// 实现了【获得请求对应的处理器和拦截器们】的骨架逻辑，而暴露 #getHandlerInternal(HttpServletRequest request) 抽象方法，交由子类实现。
public abstract class AbstractHandlerMapping extends WebApplicationObjectSupport
	...
	// 初始化拦截器
	// 该方法是对 WebApplicationObjectSupport 的覆写，而 WebApplicationObjectSupport 的继承关系是
	...
	protected void initInterceptors() {
		if (!this.interceptors.isEmpty()) {
			...
			for (int i = 0; i < this.interceptors.size(); i++) {
				...
				this.adaptedInterceptors.add(adaptInterceptor(interceptor));  // <x> 调用 #adaptInterceptor(Object interceptor) 方法，将 interceptors 初始化成 HandlerInterceptor 类型
			}
		}
	}
	...
	public final HandlerExecutionChain getHandler(HttpServletRequest request) throws Exception {
		...
		// <7> 返回 <5> 处创建的 HandlerExecutionChain 对象
